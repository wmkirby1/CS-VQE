...
summary:  trotterise things as sparse matrices - for small hamiltonians only (!)
@author: andrew
'''
...
def oplistToMatrices(oplist):
    ...
    listMatrices = []
    for op in oplist:
        listMatrices.append(sparseFermions.commutingOplistToMatrix(op))
    return listMatrices
...
def trotterise(listTerms,ordering,t,trotterNumber,trotterOrder=1):
    ...
    fullUnitary = scipy.sparse.identity(unitaryShape,format='csc')
