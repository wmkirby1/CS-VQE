...
def exponentiatePauliString(inputString):
    ...
    warnings.warn('exponentiatePauliString doesn\'t do what it says it does - make sure that the oplists you get are correct for the input.  Or use rotationFromOpterm which behaves more sensibly.')
...
def rotationFromOpterm(opterm,angle):
    from yaferp.general import fermions
    '''returns e^{-i [angle]/2 [opterm] as an oplist'''
    newOpterm = [opterm[0]*angle/2.,opterm[1]]
    coefficient = newOpterm[0]
    pauliString = newOpterm[1]
    numQubits = len(pauliString)
    sinCoefficient = -1. * 1.0j * numpy.sin(coefficient)
    cosCoefficient = numpy.cos(coefficient)
    firstTerm = [sinCoefficient, pauliString]
    secondTerm = [cosCoefficient, [0]*numQubits]
    output = [firstTerm,secondTerm]
    result = fermions.oplistRemoveNegligibles(output,1e-14)
    return result
...
'''##### OPERATOR APPLICATION WRAPPER FUNCTIONS ######'''
...
def oneXOnState(state,xIndex, useC=1):
    ...
    if useC:
        if not (type(state) is scipy.sparse.coo_matrix):
            state = scipy.sparse.coo_matrix(state)
        indices = state.row
        import oneXOnState
        state.row = oneXOnState.oneXOnState(indices,xIndex)  
        return state
...
def oneZOnState(state,zIndex, useC=1):
   ...
   # if method:
    ...
    if useC:
        ...
        indices = state.row
        import oneZOnState
        state.data = oneZOnState.oneZOnState(data,indices,zIndex)        
    return state
...
def trotteriseOplist(oplist,t,n,order):
...
def trotterise(oplist,t,n, order, groundState,alreadyTrotterised=0):
    '''calculate the unitary evolution operator corresponding to a sum
    of hamiltonian operators given as list termList, using a 
    ...
    '''
    if not alreadyTrotterised:
        trotterisedOplist = trotteriseOplist(oplist,t,n,order)
    else:
        trotterisedOplist = oplist
    groundState2 = copy.deepcopy(groundState)
    stateVector = scipy.sparse.coo_matrix(groundState2)
    for i in range(n):
        for oplist in reversed(trotterisedOplist): #right to left
...
def overlap(leftState,rightState):
    return (leftState.H * rightState)

def postUnitaryEigenvalue(finalState,initialState,t):
    overlapp = overlap(initialState,finalState)
    try:
        overlapp = overlapp.todense()
    except:
        pass
    return -1 * (angleZeroInterval(overlapp)/t)
...
def getTrueEigensystem(oplist,numEigenvalues=1,verbose=0):
    hamMatrix = commutingOplistToMatrix(oplist,boolPrintDebugInfo=verbose)
...
def oneStepTrotterError(ham,t=0,order=1,boolGroupTerms=False,maxN=20,eigenvalue=None,eigenvec=None):    
    if eigenvalue is None or eigenvec is None:
        try:
            eigenvalue, eigenvec = getTrueEigensystem(ham)
        except:
            print("Couldn't calculate eigensystem of hamiltonian")
    if not t:
        t = calculateTime(ham,eigenvalue)
    finalState = trotterise(ham,t,1,order,eigenvec)
    trotterEigenvalue = postUnitaryEigenvalue(finalState,eigenvec,t)
    difference = numpy.linalg.norm(numpy.linalg.norm(eigenvalue)-numpy.linalg.norm(trotterEigenvalue))
    return difference

def trotterError(ham,trotterSteps, t=0,order=1,boolGroupTerms=False,maxN=20,eigenvalue=None,eigenvec=None):    
    if eigenvalue is None or eigenvec is None:
        try:
            eigenvalue, eigenvec = getTrueEigensystem(ham)
        except:
    ...
    if not t:
        t = calculateTime(ham,eigenvalue)
    finalState = trotterise(ham,t,trotterSteps,order,eigenvec)
    trotterEigenvalue = postUnitaryEigenvalue(finalState,eigenvec,t)
    difference = numpy.linalg.norm(numpy.linalg.norm(eigenvalue)-numpy.linalg.norm(trotterEigenvalue))
    return difference
def scanTimeOneStep(ham,minT,maxT,tStep, eigval=None, eigvec=None):
    timeErrors = {}
    if eigval is None or eigvec is None:
        try:
            eigval, eigvec = getTrueEigensystem(ham)
        except:
            print("Couldn't calculate eigensystem of hamiltonian")
    for time2 in range(1,36):
        time = float(time2)/10
        err = oneStepTrotterError(ham,t=time,eigenvalue=eigval,eigenvec=eigvec)
        timeErrors[time] = err
    return timeErrors

def scanTime(ham,numTrotter,minT,maxT,tStep, eigval=None, eigvec=None):
    timeErrors = {}
    if eigval is None or eigvec is None:
        try:
            eigval, eigvec = getTrueEigensystem(ham)
            ...
            print("Couldn't calculate eigensystem of hamiltonian")
    for time2 in range(int(minT*10),int(maxT*10)):
        time = float(time2)/10
        err = trotterError(ham,numTrotter,t=time,eigenvalue=eigval,eigenvec=eigvec)
        timeErrors[time] = err
    return timeErrors
...
def errorByTrotterNumber(ham,t=0,order=1,listTrotterNumbers=[1],boolGroupTerms=False):    
    ...
    for thisN in listTrotterNumbers:
        ...
        trotterEigenvalue = postUnitaryEigenvalue(groundState,unitary,t)
...
def diagonalOplist(oplist):
    result = [x for x in oplist if (not 1 in x[1]) and (not 2 in x[1])]
    return result