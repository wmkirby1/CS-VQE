...
def trotterErrorForAllPermutations(hamiltonianOplist,listPermutations=None):
    ...
    return errorDict
def trotterErrorForAllPermutationsPregroup(hamiltonianOplist,pregroupOplist,trotterOrder = 1,listPermutations=None):
    numTerms = len(hamiltonianOplist)
    if listPermutations == None:
        listPermutations = generateOrderingLabels(numTerms)
    errorDict = {}
    for permutation in listPermutations:
        ...
        error = sparseFermions.oneStepTrotterError(fullOplist, order=trotterOrder)
        errorDict[tuple(permutation)] = error
    return errorDict    
