...
def controlledU(compState,groundState,oplist,t,n,order,power=1):
    #simulationTime = t/( 2 * numpy.pi)
    simulationTime = t
    unitary = sparseFermions.trotterise(oplist, simulationTime, n, order)
...
def phaseGate(compReg,listBits,numBits):
    ...
    return compReg
def iterativePhaseEstimation(oplist,groundState,tZero,numTrotter,trotterOrder,numBits):
...
def ipeaError(oplist,groundState,trueEigenvalue,tZero,trotterNumber,trotterOrder,numBits):
    readoutDigits = iterativePhaseEstimation(oplist,groundState,tZero,trotterNumber,trotterOrder,numBits)
...
def errorOfTZero(oplist,groundState,trueEigenvalue,minTZero,maxTZero,intervalTZero,trotterNumber,trotterOrder,numBits):
...
def thing(hamiltonian,state,listStuff):
    newListStuff = []
    for oneStuff in listStuff:
        oneStuff2 = copy.deepcopy(oneStuff)
        ordering = oneStuff2[0]
        tZero = oneStuff2[1]
        error = oneStuff2[2]
        thisHamiltonian = copy.deepcopy(hamiltonian)
        thisHamiltonian = commutationSubdivision.reorderOplist(thisHamiltonian, ordering)
        desiredBits = iterativePhaseEstimation(thisHamiltonian,state,tZero,2,1,11)
        qubits = ipeaQubits(thisHamiltonian,state,tZero,2,1,11)
        successProbs = ipeaQubitStringSuccessProbabilities(qubits,desiredBits)
        worstQubit = min(successProbs)[0,0]
        newListStuff.append([ordering,tZero,error,worstQubit])
