...
def kronProductList(listMatrices):
    product = numpy.mat(1)
    for thisMatrix in listMatrices:
        product = numpy.mat(scipy.linalg.kron(product,thisMatrix))
    return product
        

def symPauliToMatrix(whichPauli):
    if whichPauli == 1:
        matrix = numpy.mat([[0.0,1.0],[1.0,0.0]])
    elif whichPauli == 2:
        matrix = numpy.mat([[0.,0.-1.j],[0.+1.j,0.]]) 
    elif whichPauli == 3:
        matrix = numpy.mat([[1.,0.],[0.,-1.]])
    else:
        matrix = numpy.mat([[1.,0.],[0.,1.]])
    return matrix

def pauliStringToListMatrices(pauliString):
    '''take a list indicating a string of pauli matrices, return those matrices as a list.'''
    listMatrices = []
    for thisPauli in pauliString:
        thisMatrix = symPauliToMatrix(thisPauli)
     ...
     #   print('overallMatrix memory map created.  Calculating individual matrix terms.')
      #  sys.stdout.flush()
        ...
        for index, term in enumerate(oplist):
            ...
            coefficient = term[0]
            pauliString = term[1]
            listPauliMatrices = pauliStringToListMatrices(pauliString)
            ...
            tensoredTerm = kronProductList(listPauliMatrices)
...
def getTrueEigensystem(oplist,numEigenvalues=1):
    hamMatrix = commutingOplistToMatrix(oplist)
    eigenvalue,rawVector = scipy.linalg.eig(hamMatrix)
    #eigenvector = (rawVector[:, 0]).T
    return eigenvalue, rawVector
...
def testForGoodCombinations(oplist,n):
    import itertools
    thing = itertools.combinations(range(len(oplist)),n)
    for item in thing:
        print(str(item))
        for newitem in item:
            newList = []
            newList.append(oplist[newitem])
        eigval,eigvec = getTrueEigensystem(newList)
        print(str(numpy.matrix.max(numpy.mat(eigvec))))
    return
